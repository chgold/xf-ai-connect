...
class Manifest extends AbstractService
{
    ...
    public function generate()
    {
        ...
        if (class_exists('\chgold\AIConnectPro\Module\ProModule', false)
            && defined('\chgold\AIConnectPro\Module\ProModule::BUNDLE_REGISTRARS')
        ) {
            // Derive the bundle list from ProModule::BUNDLE_REGISTRARS — the single
            // place tools are actually registered — so this can never drift from
            // reality again. (Previously this was a hardcoded 8-item list that
            // fell behind as Pro grew to its full bundle set and even listed a
            // phantom 'automation' bundle that no longer exists.)
            $manifest['bundles_available'] = array_keys(
                \chgold\AIConnectPro\Module\ProModule::BUNDLE_REGISTRARS
            );
        } elseif (class_exists('\chgold\AIConnectPro\License\Validator', false)) {
            // Fallback (Pro installed but ProModule not loadable here): minimal set.
            $manifest['bundles_available'] = ['core'];
        }
