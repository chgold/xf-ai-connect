...
namespace chgold\AIConnectModeration\License;

/**
 * Independent licence validator for the Moderation add-on.
 *
 * Mirrors the Admin validator (chgold\AIConnectAdmin\License\Validator) exactly, but
 * validates the Moderation product's OWN licence: EXPECTED_SLUG = 'xenforo-moderation-pro',
 * with its own option keys so the Moderation licence cache never collides with Admin's or Pro's.
 *
 * Moderation is a STANDALONE product: it depends only on the free Core add-on and does
 * NOT require Pro or Admin. Without a valid Moderation licence (or the dev/test env override)
 * the Moderation tools never load — same fail-closed behaviour as Admin.
 */
class Validator
{
    private const API_URL     = 'https://goldnat.ai/api/licenses/public/validate';
    private const EXPECTED_SLUG = 'xenforo-moderation-pro';
    private const OPTION_KEY  = 'aiconnect_moderation_license_key';
    private const STATUS_KEY  = 'aiconnect_moderation_license_status';
    private const CHECKED_KEY = 'aiconnect_moderation_license_checked';

    /**
     * Server verdict when the license belongs to a different plugin/platform
     * (e.g. a Pro or Admin license presented by this Moderation add-on). Treated
     ...
     * E_WARNING on an undefined key, which would break Moderation tool loading.
     *
    ...
    private static function writeOptions(array $values): void
    {
        ...
        foreach ($values as $key => $value) {
            ...
            if (!$exists) {
                $db->insert('xf_option', [
                    ...
                    'validation_method'  => '',
                    'advanced'           => 1,
                    'addon_id'           => 'chgold/AIConnectModeration',
                ], false, false);
            }
        }
        ...
        // invalid key would keep granting Moderation tools for the rest of that
    ...
    public static function isValid(): bool
    {
        ...
        // "valid" verdict in place, so every Moderation tool kept loading until the
        ...
        // Require matching plugin_slug so another plugin's license (e.g. Pro or Admin)
        // can't unlock this one. Retained as defence-in-depth even though the
        // server now enforces the same rule via plugin_slug on the request.
