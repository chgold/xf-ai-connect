...
namespace chgold\AIConnectModeration\Admin\Controller;
 ...
 *   GET  aiconnect-moderation/license/check  → renders a confirm form inside an overlay
 *   POST aiconnect-moderation/license/check  → performs the remote validation, then
 ...
 *
 * Mirrors chgold\AIConnectAdmin\Admin\Controller\License exactly, but for the
 * Moderation product (its own Validator, option group, and route prefix).
 */
class License extends AbstractController
{
    ...
    {
        return $this->buildLink('options/groups', null) . 'aiconnect_moderation_license/';
    }

    public function actionCheck(ParameterBag $params)
    {
        $validator = \chgold\AIConnectModeration\License\Validator::class;
        ...
        return $this->view(
            'chgold\AIConnectModeration:License\Check',
            'aiconnect_moderation_license_check',
            $viewParams
        );
    }
