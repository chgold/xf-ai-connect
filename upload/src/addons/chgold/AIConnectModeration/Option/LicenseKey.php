...
namespace chgold\AIConnectModeration\Option;

use XF\Entity\Option;
use XF\Option\AbstractOption;

/**
 * Callback renderer for the aiconnect_moderation_license_key option.
 *
 * Renders inside the Options page (Setup → Options → AI Connect Moderation License)
 ...
 *   - "Check license now" button (fires JS → /admin/aiconnect-moderation/license/check)
 ...
 *   2. XF persists the key to xf_option.aiconnect_moderation_license_key via the
...
class LicenseKey extends AbstractOption
{
    ...
    {
        return '<a href="' . \chgold\AIConnectModeration\License\Validator::ACCOUNT_URL . '"'
             . ' target="_blank" rel="noopener">' . $label . ' →</a>';
    }

    public static function renderOption(Option $option, array $htmlParams): string
    {
        $status  = \chgold\AIConnectModeration\License\Validator::getStatus();
    ...
    private static function renderBadge(array $status, string $key): string
    {
        ...
        return match ($s) {
            ...
            'valid_no_updates'=> '<span class="u-flexNoShrink" style="color:#f57c00;font-weight:bold">'
                                   ...
                                   . '?subject=Renew%20AI%20Connect%20Moderation%20updates">'
            ...
            'plugin_mismatch' => '<span style="color:#c62828;font-weight:bold">❌ Wrong product</span>'
                                  ...
                                  . (($status['plugin_slug'] ?? '') !== ''
                                      ...
                                      : 'a different product')
                                  . ' and cannot enable AI Connect Moderation for XenForo.'
    ...
    private static function renderCheckButton(): string
    {
        ...
        $url = \XF::app()->router('admin')->buildLink('aiconnect-moderation/license/check');
    ...
    public static function verifyOption(&$value, Option $option): bool
    {
        ...
        //
        // The prefix is PRODUCT-specific and set server-side by goldnat.ai — Moderation
        // keys ship with their own prefix. We therefore validate only the STRUCTURE
        // (a 2-4 char alphanumeric prefix + four 4-char groups) and let goldnat.ai be
        // the authority on whether the key itself is valid.
