...
namespace chgold\AIConnectAdmin\Option;

use XF\Entity\Option;
use XF\Option\AbstractOption;

/**
 * Callback renderer for the aiconnect_admin_license_key option.
 *
 * Renders inside the Options page (Setup → Options → AI Connect Admin License)
 ...
 *   - "Check license now" button (fires JS → /admin/aiconnect-admin/license/check)
 ...
 *   2. XF persists the key to xf_option.aiconnect_admin_license_key via the
...
class LicenseKey extends AbstractOption
{
    ...
    {
        return '<a href="' . \chgold\AIConnectAdmin\License\Validator::ACCOUNT_URL . '"'
             . ' target="_blank" rel="noopener">' . $label . ' →</a>';
    }

    public static function renderOption(Option $option, array $htmlParams): string
    {
        $status  = \chgold\AIConnectAdmin\License\Validator::getStatus();
        ...
        return match ($s) {
            ...
            'plugin_mismatch' => '<span style="color:#c62828;font-weight:bold">❌ Wrong product</span>'
                                  ...
                                  . (($status['plugin_slug'] ?? '') !== ''
                                      ...
                                      : 'a different product')
                                  . ' and cannot enable AI Connect Admin for XenForo.'
    ...
    private static function renderCheckButton(): string
    {
        ...
        $url = \XF::app()->router('admin')->buildLink('aiconnect-admin/license/check');
