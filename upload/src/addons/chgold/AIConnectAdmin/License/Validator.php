...
namespace chgold\AIConnectAdmin\License;

/**
 * Independent licence validator for the Admin add-on.
 *
 * Mirrors the Pro validator (chgold\AIConnectPro\License\Validator) exactly, but
 * validates the Admin product's OWN licence: EXPECTED_SLUG = 'xenforo-addon-admin',
 * with its own option keys so the Admin licence cache never collides with Pro's.
 *
 * Admin is a STANDALONE product: it depends only on the free Core add-on and does
 * NOT require Pro. Without a valid Admin licence (or the dev/test env override) the
 * Admin tools never load — same fail-closed behaviour as Pro.
 */
class Validator
{
    private const API_URL     = 'https://goldnat.ai/api/licenses/public/validate';
    private const EXPECTED_SLUG = 'xenforo-addon-admin';
    private const OPTION_KEY  = 'aiconnect_admin_license_key';
    private const STATUS_KEY  = 'aiconnect_admin_license_status';
    private const CHECKED_KEY = 'aiconnect_admin_license_checked';

    /**
     * Server verdict when the license belongs to a different plugin/platform
     * (e.g. a Pro or WordPress license presented by this Admin add-on). Treated
     ...
     * E_WARNING on an undefined key, which would break Admin tool loading.
     *
    ...
    private static function writeOptions(array $values): void
    {
        ...
        foreach ($values as $key => $value) {
            ...
            if (!$exists) {
                $db->insert('xf_option', [
                    ...
                    'validation_method'  => '',
                    'advanced'           => 1,
                    'addon_id'           => 'chgold/AIConnectAdmin',
                ], false, false);
            }
        }
        ...
        // invalid key would keep granting Admin tools for the rest of that
    ...
    public static function isValid(): bool
    {
        ...
        // "valid" verdict in place, so every Admin tool kept loading until the
        ...
        // Require matching plugin_slug so another plugin's license (e.g. Pro)
     ...
     *   - ["appearance",...] → license grants ONLY these bundles.
